...
param_grid = {
    ...
    'min_child_weight': [1, 3, 5], #data points present in each node of tree
    'subsample': [0.7, 0.85, 1.0], #number of observations used build each tree (larger = more complex)
    'colsample_bytree': [0.7, 0.85, 1.0], #number of features used to build tree
...
grid_search = RandomizedSearchCV(xgb_model, param_grid, cv=10, scoring=f1_macro, n_iter=100, n_jobs=-1, verbose=2, random_state=64)
